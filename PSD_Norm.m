...
%% Normalization 
norm_var1 = (Table_Feat.RMS - min(Table_Feat.RMS))/(max(Table_Feat.RMS)-min(Table_Feat.RMS));
Table_Scale.RMS = norm_var1; 
 
norm_var2 = (Table_Feat.Variance - min(Table_Feat.Variance))/(max(Table_Feat.Variance)-min(Table_Feat.Variance));
Table_Scale.Variance = norm_var2; 
 
norm_var3 = (Table_Feat.Mean - min(Table_Feat.Mean))/(max(Table_Feat.Mean)-min(Table_Feat.Mean));
Table_Scale.Mean = norm_var3; 
 
norm_var4 = (Table_Feat.STD - min(Table_Feat.STD))/(max(Table_Feat.STD)-min(Table_Feat.STD));
Table_Scale.STD = norm_var4; 
 
norm_var5 = (Table_Feat.Kurtosis - min(Table_Feat.Kurtosis))/(max(Table_Feat.Kurtosis)-min(Table_Feat.Kurtosis));
Table_Scale.Kurtosis = norm_var5; 
 
norm_var6 = (Table_Feat.Skewness - min(Table_Feat.Skewness))/(max(Table_Feat.Skewness)-min(Table_Feat.Skewness));
Table_Scale.Skewness = norm_var6; 
 
norm_var7 = (Table_Feat.Peak - min(Table_Feat.Peak))/(max(Table_Feat.Peak)-min(Table_Feat.Peak));
Table_Scale.Peak = norm_var7; 
 
norm_var8 = (Table_Feat.CrestFactor - min(Table_Feat.CrestFactor))/(max(Table_Feat.CrestFactor)-min(Table_Feat.CrestFactor));
Table_Scale.CrestFactor = norm_var8; 
 
norm_var9 = (Table_Feat.SNR - min(Table_Feat.SNR))/(max(Table_Feat.SNR)-min(Table_Feat.SNR));
Table_Scale.SNR = norm_var9; 
 
norm_var10 = (Table_Feat.Entropy - min(Table_Feat.Entropy))/(max(Table_Feat.Entropy)-min(Table_Feat.Entropy));
Table_Scale.Entropy = norm_var10; 
 
norm_var11 = (Table_Feat.Energy - min(Table_Feat.Energy))/(max(Table_Feat.Energy)-min(Table_Feat.Energy));
Table_Scale.Energy = norm_var11; 
...
data=table(PC1, PC2, PC3, class_labels);
 
%% -------------- Building Classifier ----------------------------
 
classification_model=fitcecoc(data,'class_labels~PC1+PC2+PC3');
